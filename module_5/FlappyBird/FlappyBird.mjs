...
export const SpriteInfoList = {
  ...
  obstacle2:    { x:    -104, y:   0, width:   52, height: 320, count:  4 }, //fortsette med implementering av night sprites obstacle 2 er nattsprite av piper
  background:   { x:  246, y:   0, width:  576, height: 512, count:  2 },
  background2:  { x:  820, y:   0, width:  576, height: 512, count:  2 }, 
...
export const GameProps = {
  ...
  score: 0,
  bestScore: 0,
  sounds: {countDown: null, food: null, dead: null, running: null, gameOver: null, heroIsDead: null, flap: null},
};
...
function playSound(aSound) {
  if (!GameProps.soundMuted) {
    ...
    let playCheck = aSound.play();
    if (playCheck !== undefined) {
      playCheck.catch(error => console.warn(error));
    }
...
function loadGame(){
  ...
  GameProps.background2 = new libSprite.TSprite(spcvs, SpriteInfoList.background2, pos);
  GameProps.obstacles2 = new libSprite.TSprite(spcvs, SpriteInfoList.obstacle2, pos);
...
function drawGame() {
  ...
  if (GameProps.dayTime) {
    ...
    GameProps.background2.draw();
    GameProps.obstacles2.draw();
  }
...
function drawObstacles(){
  for(let i = 0; i < GameProps.obstacles.length; i++) {
    const obstacle2 = GameProps.obstacles[i];
    obstacle2.draw();
  }
}
...
function animateGame() {
  ...
  case EGameStatus.gameOver:
    ...
    if (delBaitIndex >= 0) {
      ...
      const foodSound = GameProps.sounds.food;
      foodSound.stop();
      foodSound.play();
        console.log("eat"); // brukte for å debugge lyd som bare spillte av en gang.
...
function onKeyDown(aEvent) {
  switch (aEvent.code) {
    case "Space":
      if (!GameProps.hero.isDead) {
        GameProps.hero.flap()
        const flapSound = GameProps.sounds.flap;
        flapSound.stop();
        flapSound.play(); // eneste som funket på min browser, stoppe lyden og spille, ellers blir den spillt bare en gang
